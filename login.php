...
class login_process extends front_base {
	...
	*/
	public function login(){
		...
		$this->load->model('membermodel');
		$this->membermodel->set_member_device($params['member_seq'], $_POST['device_token'], $_POST['device_type']);
	...
	*/
	...
	public function findid(){
		...
		$sql = "SELECT A.*, AES_DECRYPT(UNHEX(A.email), '{$key}') as email,
				...
				WHERE (A.rute = 'none' OR (A.rute !='none' and A.sns_change=1)) AND (A.user_name = '{$_POST[user_name]}' OR B.bname = '{$_POST[user_name]}') AND {$enc_qry} ";
